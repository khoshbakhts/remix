...
contract PuzzleNFT is ERC721URIStorage, Ownable {
    ...
    function setPuzzleGameContract(address _puzzleGameContract) public onlyOwner {
    ...
    function _removeTokenFromOwnerArray(address owner, uint256 tokenId) private {
    ...
    function ownerMintToken(address to, uint256 specificImageId) public onlyOwner {
        require(specificImageId > 0 && specificImageId <= TOTAL_IMAGES, "Invalid image ID.");
        ...
        string memory tokenUri = string(abi.encodePacked("https://qold.bitra.market/wp-includes/puzzle/", uint2str(specificImageId), ".json"));
        _setTokenURI(tokenId, tokenUri);
    ...
    function getRandomImageId(address user) internal view returns (uint256) {
        uint256 randomHash = uint256(keccak256(abi.encodePacked(block.timestamp, block.difficulty, user)));
